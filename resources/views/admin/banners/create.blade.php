...
<div class="mb-6">
    ...
    <h1 class="text-xl font-bold text-gray-800 mt-2">Nuevo banner</h1>
</div>
    ...
    <form method="POST" action="{{ route('admin.banners.store') }}" enctype="multipart/form-data" class="space-y-5">
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">
                Imagen <span class="text-gray-400">(máx. 8MB, se muestra a pantalla completa)</span>
            </label>
            <input type="file" name="imagen" required accept="image/jpg,image/jpeg,image/png,image/webp" data-preview="previewImagenBanner"
        ...
        <div>
            ...
            <textarea name="subtitulo" rows="2" maxlength="255"
                      class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:border-red-400 focus:ring-1 focus:ring-red-400">{{ old('subtitulo') }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="boton_texto" value="{{ old('boton_texto') }}" maxlength="50"
                       class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:border-red-400 focus:ring-1 focus:ring-red-400">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">
                ...
                <input type="text" name="boton_enlace" value="{{ old('boton_enlace') }}" placeholder="/tienda" maxlength="255"
                       class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:border-red-400 focus:ring-1 focus:ring-red-400">
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="datetime-local" name="publicar_en" value="{{ old('publicar_en') }}"
                       class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:border-red-400 focus:ring-1 focus:ring-red-400">
        ...
        <label class="flex items-center gap-2 text-sm text-gray-700">
            <input type="checkbox" name="activo" value="1" {{ old('activo', true) ? 'checked' : '' }} class="accent-red-600 h-4 w-4">
            Banner activo
        </label>
